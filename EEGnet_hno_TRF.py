...
#从文件夹中提取数据，构成数据集
pathlist = []
for root, dirs, files in os.walk("W:\hno\science\TRFinNoise\Rohdaten\Daten"):
	for file in files:
		if file.endswith("hdf5"):
			pathlist.append(os.path.join(root, file))

#dataset = np.empty([290304000,1],dtype = float)
...
X = dataset # format is in (trials, channels, samples)
y = y_labels
# print("type and shape of dataset x and y", type(X),type(y),X.shape, y.shape)	###查看一下数据集的结构和类型

kernels, chans, samples = 1, 32, 180

# take 50/25/25 percent of the data to train/validate/test
X_train		 = X[0:25200,]
Y_train		 = y[0:25200]
X_validate	 = X[25200:37800,]
Y_validate	 = y[25200:37800]
X_test		 = X[37800:,]
Y_test		 = y[37800:]
